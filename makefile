...
WILDCARD_DIR=$(foreach d,$(wildcard $(1:=/*)),$(if $(wildcard $d/*),$d))
WILDCARD_DIR_R=$(foreach d,$(call WILDCARD_DIR,$1),$d $(call WILDCARD_DIR_R,$d))
GET_SUB_DIRECTORIES=$(call WILDCARD_DIR_R,${PROJECT_DIRECTORY}$1)
# makes all slashes into forward slashes and removes trailing slashes
FIX_PATH_MAKE=$(patsubst %/,%,$(subst \,/,$1))
ifeq ($(OS),Windows_NT)
	SHELL=cmd.exe
...
OBJECT_OUT_DIRECTORY:=$(call FIX_PATH_MAKE,${OBJECT_OUT_DIRECTORY})
PROJECT_OUT_DIRECTORY:=$(call FIX_PATH_MAKE,${PROJECT_OUT_DIRECTORY})
SOURCE_DIRECTORIES:=$(call FIX_PATH_MAKE,${SOURCE_DIRECTORIES})
NON_RECURSIVE_SOURCE_DIRECTORIES:=$(call FIX_PATH_MAKE,${NON_RECURSIVE_SOURCE_DIRECTORIES})
LIBRARY_PREFIX:=$(call FIX_PATH_MAKE,${LIBRARY_PREFIX})
GIT_LIB_PREFIX:=$(call FIX_PATH_MAKE,${GIT_LIB_PREFIX})
INCLUDE_DIRECTORIES:=$(call FIX_PATH_MAKE,${INCLUDE_DIRECTORIES})
LIB_DIRECTORIES:=$(call FIX_PATH_MAKE,${LIB_DIRECTORIES})
SOURCE_FILES_NO_GRAPHICS:=$(call FIX_PATH_MAKE,${SOURCE_FILES_NO_GRAPHICS})

OBJECT_FILES_NO_GRAPHICS:=$(patsubst ${PROJECT_DIRECTORY}%,${PROJECT_DIRECTORY}${OBJECT_OUT_DIRECTORY}%,$(patsubst %.cpp,%.o,${SOURCE_FILES_NO_GRAPHICS}))
INCLUDE_DIRECTORIES:=$(addprefix -I ,${INCLUDE_DIRECTORIES})
LIB_DIRECTORIES:=$(addprefix -L ,${LIB_DIRECTORIES})

# these have a / at the end so that they follow the target rules
EXPANDED_SOURCE_DIRECTORIES:=$(patsubst %,$(PROJECT_DIRECTORY)/%,$(NON_RECURSIVE_SOURCE_DIRECTORIES)) $(call GET_SUB_DIRECTORIES,${SOURCE_DIRECTORIES}) $(patsubst %,$(PROJECT_DIRECTORY)%,$(SOURCE_DIRECTORIES))
BIN_DIRECTORIES:=$(addsuffix /,$(patsubst $(PROJECT_DIRECTORY)%,$(PROJECT_DIRECTORY)$(OBJECT_OUT_DIRECTORY)%,${EXPANDED_SOURCE_DIRECTORIES}))
SOURCE_FILES:=$(foreach Directory,${EXPANDED_SOURCE_DIRECTORIES},$(wildcard ${Directory}/*.cpp))
C_SOURCE_FILES:=$(foreach Directory,${EXPANDED_SOURCE_DIRECTORIES},$(wildcard ${Directory}/*.c))
...
PROJECT_FILES:=${PROJECT_DIRECTORY}${PROJECT_OUT_DIRECTORY}/lib${PROJECT_NAME}${LIB_EXTENSION} \
				${PROJECT_DIRECTORY}${PROJECT_OUT_DIRECTORY}/lib${PROJECT_NAME}-no-graphics${LIB_EXTENSION}
else
PROJECT_FILES:=${PROJECT_DIRECTORY}${PROJECT_OUT_DIRECTORY}/${PROJECT_NAME}${EXECUTABLE_EXTENSION}
endif
...
info:
	@${MAKE} ${PRINT_DIRECTORY_CHANGES} COMPILE_OS=${COMPILE_OS} BUILD_TYPE=executable BUILD_RELEASE=debug print-info
info-r:
	@${MAKE} ${PRINT_DIRECTORY_CHANGES} COMPILE_OS=${COMPILE_OS} BUILD_TYPE=executable BUILD_RELEASE=release print-info
info-lib:
	@${MAKE} ${PRINT_DIRECTORY_CHANGES} COMPILE_OS=${COMPILE_OS} BUILD_TYPE=library BUILD_RELEASE=debug print-info
info-lib-r:
	@${MAKE} ${PRINT_DIRECTORY_CHANGES} COMPILE_OS=${COMPILE_OS} BUILD_TYPE=library BUILD_RELEASE=release print-info
win:
...
help:
	...
	@echo make info: Print information about the build as debug executable
	@echo make info-r: Print information about the build as release executable
	@echo make info-lib: Print information about the build as debug library
	@echo make info-lib-r: Print information about the build as release library
ifeq (${HOST_OS},linux)
	$(call ECHO_COLOR,${COLOR_YELLOW}-----------------------------------------)
...
ifneq (${BUILD_RELEASE},UNKNOWN)
ifeq (${BUILD_TYPE},executable)
build: ${PROJECT_DIRECTORY}${PROJECT_OUT_DIRECTORY}/ ${BIN_DIRECTORIES} ${OBJECT_FILES}
...
build: ${PROJECT_DIRECTORY}${PROJECT_OUT_DIRECTORY}/ ${BIN_DIRECTORIES} ${OBJECT_FILES}
	$(call FIX_PATH,${CREATE_LIB} ${PROJECT_DIRECTORY}${PROJECT_OUT_DIRECTORY}/lib${PROJECT_NAME}${LIB_EXTENSION} ${OBJECT_FILES} ${PROJECT_FINAL_FLAGS})
	$(call FIX_PATH,${CREATE_LIB} ${PROJECT_DIRECTORY}/${PROJECT_OUT_DIRECTORY}/lib${PROJECT_NAME}-no-graphics${LIB_EXTENSION} ${OBJECT_FILES_NO_GRAPHICS} ${PROJECT_FINAL_FLAGS})
...
${PROJECT_DIRECTORY}${OBJECT_OUT_DIRECTORY}%.o:${PROJECT_DIRECTORY}%.cpp
	$(call FIX_PATH,${CPP_COMPILER} ${CPP_COMPILER_FLAGS} ${C_CPP_COMPILER_FLAGS} ${INCLUDE_DIRECTORIES} ${INCLUDE_FLAGS} ${DEP_FLAGS} -c -o ${@} ${<})

${PROJECT_DIRECTORY}${OBJECT_OUT_DIRECTORY}%.o:${PROJECT_DIRECTORY}%.c
	$(call FIX_PATH,${C_COMPILER} ${C_COMPILER_FLAGS} ${C_CPP_COMPILER_FLAGS} ${INCLUDE_DIRECTORIES} ${INCLUDE_FLAGS} ${DEP_FLAGS} -c -o ${@} ${<})

${PROJECT_DIRECTORY}${OBJECT_OUT_DIRECTORY}%/:
	-@${MKDIR} $(call FIX_PATH,${@})
...
${PROJECT_DIRECTORY}${PROJECT_OUT_DIRECTORY}/:
	-@${MKDIR} $(call FIX_PATH,${@})
	$(call ECHO_COLOR,${COLOR_YELLOW}Created Directory: ${COLOR_MAGENTA}${@})
...
clean-project-objects:
	-@${RMDIR} $(call FIX_PATH,${PROJECT_DIRECTORY}${OBJECT_OUT_DIRECTORY} ${IGNORE_STDOUT} ${IGNORE_STDERR})
	$(call ECHO_COLOR,${COLOR_GREEN}Cleaned Objects for ${COLOR_MAGENTA}${COMPILE_OS}${COMMA} ${BUILD_RELEASE})

clean-project-files: 
	-@${RM} $(call FIX_PATH,${PROJECT_FILES} ${IGNORE_STDOUT} ${IGNORE_STDERR})
...
print-info:
	...
	@echo Project Out Directory: ${PROJECT_DIRECTORY}${PROJECT_OUT_DIRECTORY}
	@echo Object Out Directory: ${PROJECT_DIRECTORY}${OBJECT_OUT_DIRECTORY}
	@echo Expanded Source Directories: ${EXPANDED_SOURCE_DIRECTORIES}
	@echo Bin Directories: ${BIN_DIRECTORIES}
	@echo C++ Source Files: ${SOURCE_FILES}
	@echo C Source Files: ${C_SOURCE_FILES}
	$(call ECHO_COLOR,${COLOR_YELLOW}-----------------------------------------)
