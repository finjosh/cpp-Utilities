...
void CommandPromptTest::test()
{
    ...
    TermFunc::Add([](TData* data){ if (data->totalTime >= 3) CommandPrompt::openPrompt(); else data->state = TState::Running; });

    // closePrompt
    TermFunc::Add([](TData* data){ if (data->totalTime >= 5) CommandPrompt::closePrompt(nullptr); else data->state = TState::Running; });
